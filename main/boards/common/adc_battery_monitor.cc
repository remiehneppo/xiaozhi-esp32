...
AdcBatteryMonitor::AdcBatteryMonitor(adc_unit_t adc_unit, adc_channel_t adc_channel, float upper_resistor, float lower_resistor, gpio_num_t charging_pin)
    ...
    // Thực hiện thiết lập có điều kiện trong phần cấu hình ADC
    if (charging_pin_ != GPIO_NUM_NC) {
        ...
        // Không thiết lập callback, để thư viện adc_battery_estimation sử dụng ước tính phần mềm
...
bool AdcBatteryMonitor::IsCharging() {
    // Ưu tiên sử dụng chức năng của thư viện adc_battery_estimation
    ...
    // Quay lại đọc GPIO hoặc trả về giá trị mặc định
    if (charging_pin_ != GPIO_NUM_NC) {
...
uint8_t AdcBatteryMonitor::GetBatteryLevel() {
    // Nếu handle không hợp lệ, trả về giá trị mặc định
    if (adc_battery_estimation_handle_ == nullptr) {
        return 100;
    }
