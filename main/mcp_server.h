...
private:
    ...
    std::optional<int> min_value_;  // Mới: giá trị nguyên tối thiểu
    std::optional<int> max_value_;  // Mới: giá trị nguyên tối đa
...
public:
    ...
    inline void set_value(const T& value) {
        // Thêm kiểm tra phạm vi cho giá trị nguyên được đặt
